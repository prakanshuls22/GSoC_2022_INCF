...
def normalize_bbco(bbox,json_obj):
    fw, fh = float(json_obj['screen_w']), float(json_obj['screen_h'])
    den = float(max(fw,fh))
...
if __name__=='__main__':
    ...
    for features in parsed_dataset.take(total_count): 
        ...
        left_bbx = normalize_bbco([lx, ly, lw, lh],features)
        right_bbx = normalize_bbco([rx, ry, rw, rh],features)        
    ...
    path_csv = '/home/skrishna/projects/def-skrishna/shared/v0_10individuals_data.csv'
