...
def normalize_bbco(bbox,fw,fh):
    ...
    x2 = (float(bbox[2]) + float(bbox[0]) - 0.5 * fw) / den
    y2 = (float(bbox[3]) + float(bbox[1]) - 0.5 * fh) / den
...
if __name__=='__main__':
    ...
    for features in parsed_dataset.take(total_count): 
        ...
        rx1, ry1, rx2, ry2 = int(features['leye_x1']), int(features['leye_y1']), int(features['leye_x2']), int(features['leye_y2'])
        lx1, ly1, lx2, ly2 = int(features['reye_x1']), int(features['reye_y1']), int(features['reye_x2']), int(features['reye_y2'])
...
#        eye_dist = (np.linalg.norm(np.array((lx_c_point,ly_c_point))-np.array((rx_c_point,ry_c_point))))  ##undivide later if neccessary
        eye_dist=abs(rx_c_point-lx_c_point) #
        width = eye_dist/1.5  ### abs in line above is key since using google code, but perhaps their sign conv. is flipped
        height=width/2

        l_lupx = lx_c_point-(width//2) #left upper
        l_lupy = ly_c_point-(height//2)
        l_rlowx = lx_c_point+(width//2) #right lower
        l_rlowy = ly_c_point+(height//2)

        r_lupx = rx_c_point-(width//2) #left upper
        r_lupy = ry_c_point-(height//2)
        r_rlowx = rx_c_point+(width//2) #right lower
        r_rlowy = ry_c_point+(height//2)

        
        #left eye crop
        # l_eye = image_vis.crop((lx_c_point-a, ly_c_point-a, lx_c_point+a, ly_c_point+a))
        l_eye = image_vis.crop((l_lupx, l_lupy, l_rlowx, l_rlowy))
        l_eye = l_eye.transpose(Image.FLIP_LEFT_RIGHT)

        #right eye crop
        r_eye = image_vis.crop((r_lupx, r_lupy, r_rlowx, r_rlowy))
    ...
    #    ry = ry + rh
        ...
        left_bbx = normalize_bbco([l_lupx, l_lupy, lw, lh],fw,fh)
        right_bbx = normalize_bbco([r_lupx, r_lupy, rw, rh],fw,fh)        
    ...
    #     print('------')
        ...
        interpreter.set_tensor(input_details[0]['index'], left_bb)
        interpreter.set_tensor(input_details[1]['index'], right_bb)
        interpreter.set_tensor(input_details[2]['index'], l_eye_final)
        interpreter.set_tensor(input_details[3]['index'], r_eye_final)
        interpreter.invoke()
    ...
    path_csv = '/home/skrishna/projects/def-skrishna/shared/vfinal_flipkey_10individuals_data.csv'
